...
本项目中涉及到相关编码格式，统一为UTF-8格式，GBK格式在我电脑上出了点问题。如果你遇到编码格式问题，请参考每个文件夹下的readme文件中的内容，在对应位置进行修改。
    下列出所有的编码格式所在
        文件 success.py ( 3数据集处理\success.py ): 42, 43, 110, 111, 114, 115
        文件 defeat.py (3数据集处理\defeat.py ): 20, 105
        文件 detect.py ( 4训练\yolov5-5.0\detect.py ): 108
        文件 train.py ( 4训练\yolov5-5.0\train.py ): 54, 56, 63, 368, 536, 560
        文件 yolo.py ( 4训练\yolov5-5.0\models\yolo.py ): 74
        文件 autoanchor.py ( 4训练\yolov5-5.0\utils\autoanchor.py ): 104
        文件 datasets.py ( 4训练\yolov5-5.0\utils\datasets.py ): 268, 369, 473, 1026, 1064
        文件 general.py (4训练\yolov5-5.0\utils\general.py): 103, 540, 549
        文件 google_utils.py ( 4训练\yolov5-5.0\utils\google_utils.py ): 91
        文件 plots.py ( 4训练\yolov5-5.0\utils\plots.py ): 323
        文件 resume.py ( 4训练\yolov5-5.0\utils\aws\resume.py ): 21
        文件 log_dataset.py ( 4训练\yolov5-5.0\utils\wandb_logging\log_dataset.py ): 11
        文件 ( D:\Program\项目文件\4训练\yolov5-5.0\utils\wandb_logging\wandb_utils.py ): 57, 75, 194, 208
