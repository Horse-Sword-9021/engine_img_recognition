...
if __name__ == '__main__':
    ...
    --name：结果保存的文件夹名称
    '''
    print("目标检测，内容识别部分，\n推理结果(识别后的图像)保存在‘4train/yolov5-5.0/runs/detect/exp[i]’系列文件夹下")
    time.sleep(3)
    path=input("请输入图片完整路径和图片文件名，\n(如VOCdevkit/images/train/1_erosion.png):")
    parser = argparse.ArgumentParser()
    parser.add_argument('--weights', nargs='+', type=str, default='weights/best.pt', help='model.pt path(s)')   # 第一次训练时的权重文件为yolov5s.pt
    parser.add_argument('--source', type=str, default=path, help='source')    # 默认参数为0时打开摄像头  # file/folder,0 for webcam
